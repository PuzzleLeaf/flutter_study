...
Either<ValueFailure<IList<T>>, IList<T>> validateMaxListLength<T>(IList<T> input, int maxLength) {
  if (input.length() <= maxLength) {
    return right(input);
  } else {
...
//  final emailAddress = EmailAddress('faseda');
//
//  String emailText = emailAddress.value.fold(
//    (left) => 'Failure Happend ${left}',
//    (right) => right,
//  );
//
//  String emailText2 = emailAddress.value.getOrElse(() => 'Failure Happend');
//}
